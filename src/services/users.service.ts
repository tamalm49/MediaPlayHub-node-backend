...
export class UsersService {
    ...
    public static async save(body: UserType, files?: any): Promise<string> {
        try {
            ...
            sendEmail({ to: [saveUser.email], text: `Hi ${saveUser.firstName}`, subject: "testing" })
            return saveUser.id;
        } catch (error) {
            throw error;
        }
    ...
    public static async update(param: {
        ...
        try {
            let user = await User.findByIdAndUpdate(param.id, param, { new: true }).select(["_id","email","userName"]);
            if (!user) {
    ...
    private static generateToken(payload:AccessPayload){
        try {
            let accessToken=  jwt.sign(payload, String(process.env.ACCESSSECRET), { algorithm: "HS512", expiresIn: "1h" });
            // let privateKey = fs.readFileSync("private.pem","utf-8");
            let refreshToken = jwt.sign(payload,String(process.env.REFRESHSECRET),{algorithm: "HS512",expiresIn: "7d"})
            return {
                accessToken,
                refreshToken
            }

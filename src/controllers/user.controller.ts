...
export class UserConntroller {
    ...
    public static async login(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            console.log(login);
            res.status(200)
            .cookie("ACCESSTOKEN",login.accessToken,{httpOnly:true,secure:true,maxAge : 1000*60*60})
            .cookie("REFRESHTOKEN",login.refreshToken,{httpOnly:true,secure:true,maxAge:1000*60*60*24*7})
    ...
    public static async logout(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            .json(new ApiRespose(200,"logout successfull"));
        } catch (error) {
            next(error);
        }
    }
